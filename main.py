...
def main(yolo5_config):
    ...
    deepsort_tracker = DeepSort(cfg.DEEPSORT.REID_CKPT, max_dist=cfg.DEEPSORT.MAX_DIST,
                                ...
                                use_cuda=False, use_appearence=True)
        ...
        if ret:
            ...
            """
                isCountPresent:
                    ...
                    False：表示显示总人数和当前人数
            """
            result_img, present_num = Counting_Processing(img, yolo5_config, Model, class_names, deepsort_tracker,
                                                          Obj_Counter, isCountPresent=False)

            # 如果返回的是异常，则跳过该帧
            if isinstance(result_img, Exception):
                print(f"处理第 {mycap.get_index()} 帧时发生异常: {result_img}")
                # 可以选择打印完整堆栈，但 Counting_Processing 已经打印了
