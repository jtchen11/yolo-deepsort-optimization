...
def increment_path(path, exist_ok=False, sep='', mkdir=False):
    """
    Increment path, i.e. runs/exp --> runs/exp{sep}2, runs/exp{sep}3, ... etc.
    If the path exists and exist_ok is not True, the path will be incremented by appending a number.
    """
    ...
    if path.exists() and not exist_ok:
        ...
        matches = [re.search(rf"{path}{sep}(\d+)", d) for d in dirs]
